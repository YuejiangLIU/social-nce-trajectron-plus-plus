...
class ModelRegistrar(nn.Module):
    ...
    def save_models(self, curr_iter, prefix=None):
        # Create the model directiory if it's not present.
        if prefix:
            save_path = os.path.join(self.model_dir,
                                 'model_registrar-{}-{:d}.pt'.format(prefix, curr_iter))
        else:
            save_path = os.path.join(self.model_dir,
                                 'model_registrar-{:d}.pt'.format(curr_iter))

        torch.save(self.model_dict, save_path)

    def load_models(self, iter_num=None, prefix=None):
        self.model_dict.clear()
        
        if prefix:
            fname = 'model_registrar-' + prefix
        else:
            fname = 'model_registrar'

        if iter_num and iter_num > 0:
            save_path = os.path.join(self.model_dir, fname + '-%d.pt' % iter_num)
        else:
            save_path = os.path.join(self.model_dir, fname + '.pt')            
